...
![G*Power Screenshot](writeup/power_calculation.png)
...
The additional attention check uses this prompt, and participants are able to select as few or as many of the options presented in random order:

> "For the purposes of this survey, we need to know that you have been paying attention to the descriptions. Can you select *every single one* of the discoveries you've read about in this survey?" [Flying Fish, Reflective Smoke, Self-Forming Glass, Glowing Rocks, Liquid Helium Weather Systems, Warm Ice, Non-Flammable Wood, Jumping Snails, Cold Energy]
...
The participants then completed the same attention check and external website question used in the original paper. We used an additional manipulation check not in the original paper where participants are asked to select the phenomena that they read about out of a list of the 6 phenomena presented and an additional 3 unseen phenomena. However, a pilot of the experiment produced <50% success rate, suggesting that perhaps the memory demands recalling the phenomena are too high. Nevertheless, we choose to include the attention check and conduct all analyses with and without exclusions on this final attention check.
...
As mentioned earlier, we have added an manipulation check that was not used in the original study, though its efficacy is in question. In addition to performing the above analyses using only the exclusions from the original paper, we will also try those analyses excluding any participant who has more than 1 error (false positive or false negative) in identifying the different fictitious phenomena used in the study. (See Materials).
...
```
###We try each of the above analyses on the non log transformed data
```{r}
